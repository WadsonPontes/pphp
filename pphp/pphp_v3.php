...
function textoSQL($t) {
	...
	while ($r = contemComparador($t)) {
		...
		if (contem($t, " ")) {
			add($campo, true, tirarEspacos(subtexto($t, contem($t, " ", true) + 1)));
			$t = tirarEspacos(subtexto($t, 0, contem($t, " ", true) - 1));
			add($logico, true, subtexto($t, contem($t, " ", true) + 1));
			$t = tirarEspacos(subtexto($t, 0, contem($t, " ", true) - 1));
		}
		else {
			add($campo, true, tirarEspacos($t));
			$t = "";
		}
	}
...
function iniciarAPI() {
	// conectar();
	echo var_dump(qtd("   nome=!'Thiago' E (descricao==Olá Mundo! OU id <> 4 )"));
}
